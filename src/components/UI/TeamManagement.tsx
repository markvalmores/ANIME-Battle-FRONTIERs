    ...
    <div className="w-full h-full bg-zinc-950 p-4 sm:p-12 flex flex-col">
      <div className="flex justify-between items-center mb-6 sm:mb-12 shrink-0">
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          <h2 className="text-xl sm:text-4xl font-bold italic">TEAM <span className="text-emerald-400">MANAGEMENT</span></h2>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-12 flex-1 overflow-y-auto lg:overflow-hidden pr-1">
        {/* Current Team */}
        <div className="flex flex-col gap-4 lg:gap-6 shrink-0 lg:overflow-y-auto pb-4 lg:pb-0">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-4">
            {saveData.team.map((charId, i) => {
              ...
              return (
                <div key={i} className="h-20 sm:h-24 bg-white/5 border border-white/10 rounded-2xl p-3 sm:p-4 flex items-center gap-4 relative overflow-hidden">
                  {char ? (
                    <>
                      <AnimeCharacter url={char.gifUrl} className="w-12 h-12 sm:w-16 sm:h-16 object-contain" />
                      <div className="flex-1 min-w-0">
                        <div className="font-bold text-sm sm:text-base truncate">{char.name}</div>
                        <div className="text-[10px] sm:text-xs text-white/50 uppercase truncate">{char.rarity}</div>
                      </div>
                      <button 
                        onClick={() => toggleTeamMember(char.id)}
                        className="p-1.5 sm:p-2 bg-red-500/10 text-red-500 rounded-lg hover:bg-red-500/20 transition-colors shrink-0"
                      >
                        <Minus size={16} className="sm:size-[20px]" />
                      </button>
                    </>
                  ) : (
                    <div className="flex-1 flex items-center justify-center text-white/20 border-2 border-dashed border-white/5 rounded-xl h-full text-xs font-bold">
                      EMPTY SLOT
                    </div>
                  )}
                </div>
        ...
        {/* Collection */}
        <div className="lg:col-span-2 flex flex-col gap-4 lg:gap-6 lg:overflow-hidden">
          <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center shrink-0">
            ...
            <div className="relative w-full sm:w-auto">
              ...
              <input 
                ...
                className="bg-white/5 border border-white/10 rounded-xl py-2 pl-10 pr-4 text-sm focus:outline-none focus:border-emerald-500/50 transition-colors w-full sm:w-64"
              />
            </div>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 lg:overflow-y-auto pr-2 pb-12">
